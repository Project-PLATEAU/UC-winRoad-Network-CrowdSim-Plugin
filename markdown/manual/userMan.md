...
本書では、UC-win/Road人流シミュレーションプラグイン（以下「本システム」という。）の操作手順について記載しています。
...
・追加したCityGMLを読み込む場合、このボタンをクリックします。読み込み完了後、自動的にCityGML読み込み画面を閉じます。

⑥ 閉じる

・CityGMLを読み込まずにCityGML読み込み画面を閉じる場合、このボタンをクリックします。

⑦ ヘルプ

・CityGML読み込み画面に関するヘルプを確認する場合、このボタンをクリックします。
...
・シミュレーションの条件設定を行う場合、このボタンをクリックすると[シミュレーション設定画面](#2-3-1-シミュレーション設定画面)が表示されます。
...
・シミュレーションを実行する場合、このボタンをクリックすると[シミュレーション実行画面](#2-3-7-シミュレーション実行画面)が表示されます。
...
・シミュレーションでの歩行可能領域に関する設定を行う場合、このボタンをクリックすると[地域設定画面](#2-3-2-地域設定画面)が表示されます。
...
シミュレーションを実施する領域（以下「メッシュ」という。）の生成・編集およびメッシュに基づいて歩行者が通行できる経路（以下「歩行ネットワーク」という。）の生成を行う画面です。
...
・選択した範囲でメッシュを生成する場合、このボタンをクリックするとメッシュを生成します。生成されたメッシュは読み込んだCityGMLの属性（※1）に基づいて、マスごとに歩行可能エリアと歩行不可エリアを分類します。

⑥ 歩行可能に変更する/歩行不可に変更する

・メッシュ生成後に歩行可能エリア・歩行不可エリアの編集を行う場合、いずれかのボタンが選択されている状態でUCwinRoadメイン画面の地形をクリックすると、クリックした地点のマスを歩行可能エリアまたは歩行不可エリアとして設定します。
...
・歩行ネットワークを生成する場合、このボタンをクリックするとメッシュの歩行可能エリアに基づいて歩行ネットワークが生成（※2）されます。

![](../resources/userMan/AreaSettings.png)

（※1）[3D都市モデル標準製品仕様書 第5版](https://www.mlit.go.jp/plateaudocument/#toc0_02)に準拠するCityGMLが対象となります。CityGMLの属性がLOD1の道路（tran:Road）である場合はモデルが指定する全域が、LOD3の道路（tran:Road）である場合はモデルが指定する通行可能な領域（tran:TrafficArea）が歩行可能エリアとなります。

（※2）歩行ネットワークは、歩行者が目指す地点となる「ノード」というオブジェクトと、歩行者が通行できる経路となる「パス」というノード同士を結ぶ線形によって構成されます。

![](../resources/userMan/PedestrianNetwork.png)
...
![](../resources/userMan/CrosswalkAdd.png)
...
・動作設定の内容を変更する場合は、このボタンをクリックすると歩行者動作設定画面（※）が表示され、⑥で選択した動作設定の内容を変更することができます。
...
（※）歩行者動作設定画面では、歩行者の経路選択傾向や他の歩行者と衝突する距離、歩行速度、バス乗降時及び改札通過時の所要時間を設定します。

![](../resources/userMan/PedestrianDetail.png)
...
・人流発生地点を追加する場合、このボタンをクリックすると①で選択した内容に従って人流発生・退出地点を追加するモード（※）に移行します。

②-2 人流退出地点の追加

・人流退出地点を追加する場合、このボタンをクリックすると①で選択した内容に従って人流発生・退出地点を追加するモード（※）に移行します。
...
（※）①で「通常の人流発生/退出地点」を選択した状態で②-1または②-2をクリックすると、下図のような状態になります。この状態でUCwinRoadメイン画面からノードをクリックすると、そのノードを人流発生・退出地点として設定します。
...
・「時間周期」では、人流が発生する時間の間隔を設定します。また、「詳細設定」ボタンをクリックすると時間帯別の人流発生倍率を設定することができます（※）。

・「ダイヤ」では「ダイヤを設定する」ボタンが表示され、クリックすると[ダイヤ設定画面（人流発生地点）](#2-3-4-5-ダイヤ設定画面人流発生地点)が表示されます。

・「1時間当たりの発生人数」では、「詳細設定」ボタンをクリックすると時間帯別の人流発生倍率を設定することができます（※）。
...
![](../resources/userMan/PopOutSettings02.png)

（※）「詳細設定」ボタンをクリックすると、下図のウインドウが表示されます。ここから時間帯別の人流発生倍率を変更することができます。

![](../resources/userMan/PopDetail.png)
...
![](../resources/userMan/WaitingQueueSettings02.png)
...
![](../resources/userMan/WaitingQueueAreaSelect.png)
...
・クリックすると、④に指定されているノードに待機列を追加するモード（※）に移行します。

![](../resources/userMan/WaitingQueueSettings03.png)

（※）⑭をクリックすると下図のような状態になります。「ノードの編集」画面の「＋」ボタンをクリックすると待機列の追加が開始されます。
...
![](../resources/userMan/SimulationTab.png) 
...
![](../resources/userMan/CrossSectionResult.png) 
...
![](../resources/userMan/BusTransportResult.png) 
...
![](../resources/userMan/WaitingQueueResult.png) 
...
![](../resources/userMan/BusTransportGraph.png) 
